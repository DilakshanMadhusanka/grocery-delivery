      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-app-text-light text-sm mb-4">Start ordering to see your orders here</p>
            ...
            {orders.map((order) => (
              <Link key={order._id} to={`/orders/${order._id}`} className="block max-w-4xl bg-white rounded-2xl p-5 hover:shadow transition-all">
                ...
                <div className="flex justify-between items-start mb-3">
                  {/* left */}
                  <div>
                    <p className="font-medium text-app-green text-sm">Order #{order._id.slice(-8).toUpperCase()}</p>
                ...
                <div className="flex items-centergap-2 mb-3">
                  {order.items.slice(0, 4).map((item, i) => (
                    <img
                      ...
                      src={item.image}
                      alt={item.name}
                      className="size-12 sm:size-16 object-cover rounded-lg border border-app-border"
                    />
                  ))}

                  {order.items.length > 4 && (
                    <div className="size-12 sm:size-16 bg-app-cream flex-center rounded-lg text-app-text-light text-xs font-semibold">
                      +{order.items.length - 4}
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center text-sm pt-3">
                  <span className="text-app-text-light">{order.items.length} {order.items.length === 1 ? "item" : "items"}</span>
